...
public class MemoryStressor {
     ...
     */
    public void allocate(int targetPercent,boolean operatingSystemMemoryCheck,int systemMemPercent ) {
        if (allocated) {
            return;
        }
        // 检查系统内存压力
        ...
        while ((currentUsed = rt.totalMemory()  - rt.freeMemory())  < targetBytes) {
            try {
                ...
                // 填充数据确保内存被实际使用
                byte[] chunk = memoryBlocks.get(memoryBlocks.size() - 1);
    ...
    private boolean isMemoryPressureHigh(int systemMemPercent,int targetPercent) {
        ...
        try {
            ...
            if (osBean instanceof com.sun.management.OperatingSystemMXBean unixBean) {
                long freeMem = unixBean.getFreeMemorySize();
                long totalMem = unixBean.getTotalMemorySize();
